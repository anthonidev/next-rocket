...
import { Plan } from '../../redux/slice/serviceSlice'
...
const Planes: FunctionComponent<{
    ...
    const renderPlan = (plan: Plan) => {
        ...
        return plan.is_recomented ? (
            <div key={plan.id} className="flex justify-center items-center  ">
                ...
                <div
                    className="hover:scale-110 scale-105 transition-all ease-in-out duration-500    flex flex-col p-6 space-y-6 rounded shadow sm:p-8 dark:bg-dark-300 bg-white hover:bg-indigo-50  ">
                    ...
                    <div className="space-y-2">
                        ...
                        <span className="text-5xl font-bold text-dark dark:text-red-500">{formatterSoles.format(plan.price)}</span>
                        <p className='flex justify-end space-x-3 text-gray-500 dark:text-red-200 text-xs '>
                            <span>Precio normal:</span>
                            <p className="  decoration-double">{formatterSoles.format(plan.compare_price)}</p>
                        </p>
                    </div>
                    <p className=" leading-relaxed dark:text-gray-400 text-right font-roboto font-bold text-gray-500">{plan.subtitle}</p>
            ...
            <div key={plan.id} className="flex justify-center items-center  ">
                <div
                    className="hover:scale-105 transition-all ease-in-out duration-500    flex flex-col p-6 space-y-6 rounded shadow sm:p-8 dark:bg-dark-300 bg-white hover:bg-indigo-50  ">
                    <div className="space-y-2">
                        <h4 className="text-2xl font-bold text-indigo-900 underline dark:text-indigo-100">{plan.name}</h4>
                        <span className="text-5xl font-bold text-dark dark:text-red-500">{formatterSoles.format(plan.price)}</span>
                    </div>
    ...
    return (
        <section id="prices" className="py-20 bg-indigo-900 dark:bg-dark dark:text-gray-100">
            <div className="container px-4 mx-auto ">
                <div className="max-w-2xl mx-auto mb-16 text-center">
                    <span className="font-bold tracking-wider uppercase text-indigo-50 dark:text-violet-400">Planes</span>
                    <h2 className="text-4xl font-bold lg:text-5xl text-white"> <span className='text-yellow-400 dark:text-red-500 underline'>Plan</span>  de {title}</h2>
                </div>
                <div className=" grid md:grid-cols-3 grid-cols-1  max-w-7xl mx-auto">
